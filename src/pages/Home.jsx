...
const Home = () => {
  ...
  const todayData = {
    ...
    percentage: consumption.dailyLimit > 0 
  ...
  const monthlyData = {
    ...
    percentage: consumption.monthlyLimit > 0 
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <p className="text-sm text-yellow-700">
              <strong>Mode démonstration :</strong> Les données de consommation sont simulées pour illustrer le fonctionnement. 
              Pour des données réelles, une application mobile ou extension navigateur serait nécessaire.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${
                  networkStats.type === '4g' ? 'bg-green-500' : 
                  networkStats.type === '3g' ? 'bg-yellow-500' : 'bg-blue-500'
                }`}></div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-gray-800">Statistiques de consommation</h2>
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg p-4 shadow">
          ...
          <div className="text-2xl font-bold text-blue-600">
            {Math.max(consumption.dailyLimit - consumption.daily, 0).toFixed(2)} Go
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-4 shadow">
          ...
          <div className="text-2xl font-bold text-blue-600">
            {Math.max(consumption.monthlyLimit - consumption.monthly, 0).toFixed(2)} Go
          </div>
        </div>
      </div>
    </div>
